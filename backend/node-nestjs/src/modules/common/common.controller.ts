...
import { ApiTags, ApiOperation, ApiQuery, ApiOkResponse, ApiBadRequestResponse, ApiBadGatewayResponse, ApiInternalServerErrorResponse } from '@nestjs/swagger';
...
export class CommonController {
  ...
  @ApiQuery({ name: 'url', description: '远程JSON数据的URL地址', required: true, type: String })
  ...
  async fetchJsonFromUrl(@Query(new ValidationPipe({ transform: true })) query: FetchJsonDto) {
    const { url } = query;
    ...
    try {
      ...
      if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
        throw new HttpException('Unable to connect to the provided URL', HttpStatus.BAD_GATEWAY);
      }
